...
CGLRenderer::CGLRenderer(void)
{
	...
	light_blue.SetAmbient(0.2, 0.0, 0.2, 0.0);
	light_blue.SetDiffuse(1.0, 0.0, 1.0, 0.0);
	light_blue.SetSpecular(0.5, 0.5, 0.5, 0.0);
	light_blue.SetShininess(25.0);

	dark_blue.SetAmbient(0.0, 0.0, 0.2, 0.0);
	dark_blue.SetDiffuse(0.0, 0.0, 1.0, 0.0);
	...
	blueSphere.SetAmbient(0.0, 0.2, 0.2, 1); //na zid na f2 
	blueSphere.SetDiffuse(0.0, 1.0, 1.0, 0); //kuglica
	...
	//crvena
	redSphere.SetAmbient(0.2, 0.0, 0.0, 1);//na f3 svetlo odozgo
	...
	//zuta
	yellowSphere.SetAmbient(0.2, 0, 0.0, 1);//NA F1
	yellowSphere.SetDiffuse(1.0, 0.0, 0.0, 0);
	yellowSphere.SetSpecular(1, 0, 0, 1);
	yellowSphere.SetEmission(1, 0, 0, 1);
...
void CGLRenderer::DrawVase()
{
	...
	//int niz[14] = { 0, -3, -2, 0, 1, 3, -1, -3, -1, 3, -1, -3, -1, -1};//3 -3 smanjuju povecavaju, 2 1 donji gornji prsten povecavaju smanjuju respektivno
//	int niz[14] = { 0,-3,-2,-1,1,-1,-1,3,-1,-3,-1,3,3,-1};//2 povecava donji ring, 1 povecava gornji ring




	//int niz[14] = { 0,-3,-2,-1,1,-1,-1,3,-1,-1,-1,-3,-1,-1 };
	int niz[14] = { 0,-3,-2,-1,1,3,3,3,-1,-1,-1,-3,-3,-3 };
...
void CGLRenderer::DrawRing(float radius1, float radius2, float height)
{
	...
	for (int i = 0; i < 180; i++)
	{
		...
		float stranica = fabsf(radius1 - radius2);
...
void CGLRenderer::YellowLight()
{
	...
	float light_ambient[] = { 0.25, 0.25, 0.0, 1.0 };
	float light_diffuse[] = { 1, 0, 0, 1.0 };
	float light_specular[] = { 1, 0, 0, 1.0 };
...
void CGLRenderer::RedLight()
{
	...
	float light_specular[] = { 0, 0, 0.5, 1.0 };
...
void CGLRenderer::BlueLight()
{
	...
	float light_diffuse[] = { 0, 1.0, 1.0, 1.0 };
