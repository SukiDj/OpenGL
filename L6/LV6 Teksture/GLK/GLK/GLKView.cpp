...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	if (nChar == VK_UP)
	{
		...
		m_glRenderer.cameraX = m_glRenderer.cameraZ * cos(m_glRenderer.angle);
		m_glRenderer.cameraY = m_glRenderer.cameraZ * sin(m_glRenderer.angle);
		Invalidate();*/
		m_glRenderer.angle -= 0.1;
	...
	{
		m_glRenderer.angle2 += 0.1;
	}
	else if (nChar == VK_RIGHT)
	{
		m_glRenderer.angle2 -= 0.1;
	}
	else if (nChar == 'Q') //normale
	{
		//m_glRenderer.tx_korak += 0.5;
		Invalidate();

		CView::OnKeyDown(nChar, nRepCnt, nFlags);
		return;
	}
	else if (nChar == 'W') //red light
	{
		//m_glRenderer.tx_korak -= 0.5;
		Invalidate();
	...
	else if (nChar == 'A') //green light
	{
		//m_glRenderer.ty_korak += 0.5;
		Invalidate();

		CView::OnKeyDown(nChar, nRepCnt, nFlags);
		return;
	}
	else if (nChar == 'S') //blue light
	{
		//m_glRenderer.ty_korak -= 0.5;
		Invalidate();
	...
	double alfa = m_glRenderer.angle;
	double beta = m_glRenderer.angle2;
	double xx = m_glRenderer.cameraX;
	double yy = m_glRenderer.cameraY;
	double zz = m_glRenderer.cameraZ;

	double zoom = 17.0;
	m_glRenderer.cameraX = zoom * cos(alfa) * cos(beta);
	m_glRenderer.cameraY = zoom * sin(alfa);
	m_glRenderer.cameraZ = zoom * cos(alfa) * sin(beta);
	m_glRenderer.angle = alfa;
	m_glRenderer.angle2 = beta;
